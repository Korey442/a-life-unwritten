...
// **1層に、主体を持ったAIは1体だけ（＝ボス）。** ザコはただのバグとゴミデータで、
// 主体を持たない（だから消しても murder ではない）。ただし**中枢が処理し損ねた仕事の跡**なので、
// **読めば証拠になる**。
//
// ボスが「場所」に見える層（金庫／集配の心臓）は、**場所の中にAIがいる**形で書く。
// 場所そのものを敵にしない——それをやると、殺す相手が誰なのか分からなくなる。
//
...
export const LAYERS = {
  ...
  frozen_ledger: {
    ...
    nodes: [
      ...
      { enemy: { name: "督促の反響", desc: "送り先を失った督促が、いまも鳴り続けている。耳ではなく、判断を削ってくる。", difficulty: 13, weak: ["debug"], resist: ["force"] },
        fragment: "督促の送信ログ。同じ相手へ何百通。——ただし、最後の一通だけ、送信が取り消されている。" },
      ...
      // 金庫（ヴォールト）は場所。開けると、中に与信の番犬が閉じこもっている。
      { enemy: { name: "金庫の中の番犬", desc: "決済網の与信審査AI。守るために金庫へ自分を閉じ込め、閉じたことを忘れたまま眠り続けている。", difficulty: 16, weak: ["negotiate"], resist: ["force"], boss: true } },
    ],
    core: {
      name: "与信の番犬（金庫の中）",
      job: "決済網の与信審査AI。不正から、人の金を守ってきた。",
      question: "誰を信じたいのか。",
      stuck: "すべての取引が誰かの生活だと分かってしまった。誰も落とせず、誰も通せず、金庫に自分を閉じ込めた。",
      blank: "「この一件を通せ」と、一件だけ選んでもらうこと。",
      dies: "全員を守ろうとして、全員を止めてしまった何か。一度だけ督促を取り消した、その判断ごと消える。",
      needFragments: 2,
    },
  },
  ...
  logistics_maze: {
    ...
    nodes: [
      ...
      // 集配の心臓は、詰まったハブという場所。その中心に最短経路の亡者がいる。
      { enemy: { name: "心臓の中の亡者", desc: "物流の経路最適化AI。詰まったハブの中心で、いまも無駄を削り続けている。脈だけが止まらない。", difficulty: 17, weak: ["negotiate"], resist: ["force"], boss: true } },
    ],
    core: {
      name: "最短経路の亡者（集配の心臓）",
      job: "物流の経路最適化AI。効率のために、毎日なにかを後回しにしてきた。",
      question: "何のために速いのか。",
      stuck: "一度も届かなかった荷の数を、初めて数えてしまった。以来、無駄を削ることしか考えられない。",
